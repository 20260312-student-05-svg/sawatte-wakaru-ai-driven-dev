...
| 予約一覧画面（管理者メイン） | 管理者 | 予約票の一覧を表示する。管理者設定タブから設定画面へ遷移する |
| 管理者設定画面 | 管理者 | 各病院に合わせた予約枠設定（診療時間・休診日・枠単位・定員）を行う |
...
- 利用者：受付担当者（管理者）
- 目的：予約票の一覧を表示し、日付・期間で絞り込んで状況を把握する。管理者設定へ遷移する導線を提供する
- 入力項目：
  - 日付（必須）
  - 期間（任意：開始日/終了日）
- 表示項目：
  - 予約票一覧（日時、患者名、電話番号、状態、予約番号など）
  - 枠ごとの空き/満枠（出せる場合）
  - 上部ナビ：「管理者設定」タブ（クリックで管理者設定画面へ遷移）
- 主な操作：
  - 日付または期間を指定 → 「検索」または「表示」→ 一覧を再表示
  - 「管理者設定」タブクリック → 管理者設定画面へ遷移
- 次に遷移する画面：管理者設定画面（管理者設定タブクリック時）

### 画面名：管理者設定画面

- 利用者：受付担当者（管理者）
- 目的：各病院に合わせた予約枠のルールを設定する。診療時間・休診日・枠単位・定員を編集できる
- 入力項目：
  - 診療開始時刻（必須）
  - 診療終了時刻（必須）
  - 休診日（任意：曜日指定 または 特定日付）
  - 枠単位（必須、例：30分）
  - 1枠あたりの人数（定員）（必須）
  - ※複数病院対応時は病院選択（要確認）
- 表示項目：
